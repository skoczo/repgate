...
func TestIPCache(t *testing.T) {
	...
	cache.Set("127.0.0.1", IPRecord{IP: "127.0.0.1", Status: "safe", Score: 0, Source: "test", CheckedAt: time.Now(), ExpiresAt: time.Now().Add(time.Hour)})
...
func TestIPCacheMaxSize(t *testing.T) {
	cache := NewIPCache(100)
	for i := 0; i < 100; i++ {
		cache.Set(fmt.Sprintf("127.0.0.%d", i), IPRecord{IP: fmt.Sprintf("127.0.0.%d", i), Status: "safe", Score: 0, Source: "test", CheckedAt: time.Now(), ExpiresAt: time.Now().Add(time.Hour)})
	}
